...
export default function AuditoriaPublica() {
  return (
    <>
      <SEOHead
        title="Auditoria Pública — Trace Hub"
        description="Transparência técnica do Trace Hub: código-fonte auditável, arquitetura de captura, ambiente sandbox e ancoragem criptográfica redundante."
        canonicalPath="/auditoria-publica"
      />
...
└──────────────┘                │  ┌────────────────────┐ │
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              <strong>Nota de escopo (achado N-05 da reanálise pericial):</strong> os módulos
              proprietários de captura (renderização Chromium sandbox) não compõem o core
              open-source publicado sob BUSL 1.1. A admissibilidade judicial é assegurada
              pelos módulos de <em>verificação independente</em> publicados — <code>tsr-verify</code>,{' '}
              <code>ots-verify</code>, <code>forensic-hash</code> e <code>originstamp-verify</code> —
              que permitem ao perito revalidar offline qualquer evidência sem depender da
