...
// Apenas resolvers DoH que respondem de forma estavel a partir do runtime serverless.
// Quad9, OpenDNS e AdGuard retornam HTTP 400 / erro de conexao daqui e poluiam o
// relatorio com linhas de falha. Consenso passa a ser 2/2 (Google x Cloudflare),
// operadores independentes — suficiente para deteccao de poisoning.
...
async function computeConsensus(a4: ResolverResult[], a6: ResolverResult[]): Promise<{
  ...
  // independentes (Google × Cloudflare).
  ...
  try {
    ...
    return new Response(
      JSON.stringify({
        ...
        method: 'DoH (RFC 8484 JSON) — Google + Cloudflare · consenso por /24·/48 + ASN (Team Cymru)',
      }),
