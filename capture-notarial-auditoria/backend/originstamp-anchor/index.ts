...
interface Body {
  ...
  /** Contexto opcional: identifica a ferramenta de origem para o acompanhamento
   *  automático da confirmação Bitcoin e o e-mail de aviso ao usuário. */
  context?: {
    tool?: string;
    ref_id?: string | null;
...
async function submitToOpenTimestamps(digest: Uint8Array): Promise<{ ots: Uint8Array; calendar: string }> {
  ...
  // Timeout de 20 s: se algum calendário travar, abortamos com erro claro em vez
  // de deixar a edge function ser morta pelo runtime.
  const STAMP_TIMEOUT_MS = 20_000;
  try {
    await Promise.race([
      // Passamos a lista curta CALENDARS (3 pools estáveis) para evitar os
      // calendários lentos padrão (catallaxy, eternitywall).
      (OpenTimestamps as any).stamp(detached, { calendars: CALENDARS, m: 2 }),
      new Promise((_, reject) =>
        setTimeout(() => reject(new Error(`stamp timeout after ${STAMP_TIMEOUT_MS}ms`)), STAMP_TIMEOUT_MS),
      ),
    ]);
  } catch (e) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Registra o acompanhamento automático da confirmação Bitcoin (best-effort).
    // O robô `check-bitcoin-anchors` faz o upgrade do .ots e avisa o usuário por
    // e-mail assim que a prova em bloco existir.
    try {
      ...
      await service.from("bitcoin_anchor_watch").upsert({
        user_id: user.id,
        tool: ctxIn.tool || "desconhecido",
        ref_id: ctxIn.ref_id ?? null,
        ref_label: ctxIn.ref_label ?? null,
        evidence_hash: hash,
        ots_base64: otsBase64,
        tsr_base64: (rfc3161 as any).token_base64 ?? null,
        status: "processing",
        notify_email: ctxIn.notify_email || user.email || null,
        last_check_at: null,
      }, { onConflict: "tool,evidence_hash" });
    } catch (watchErr) {
      console.error("[anchor-watch] falha ao registrar acompanhamento:", watchErr);
    }
