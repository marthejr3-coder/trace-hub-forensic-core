...
import { LIMITES_ESCOPO_CLAUSE } from './legal-retention';
...
export function drawResumoJuizoPDF(
  ...
  const boxH = padding + 5 + linhas.length * lineH + 6 + 14 + 16 + 14 + padding;
  ...
  ry += fundLines.length * 3.4 + 1.5;

  // Cláusula de LIMITES DE ESCOPO (integridade vs autenticidade)
  pdf.setFont('helvetica', 'bold');
  pdf.setFontSize(7.2);
  pdf.setTextColor(120, 53, 15);
  const limLines = pdf.splitTextToSize(LIMITES_ESCOPO_CLAUSE, width - padding * 2);
  pdf.text(limLines, x + padding, ry);
  ry += limLines.length * 3.4;
